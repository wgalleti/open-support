...
class Attendant (models.Model):
    access_user = models.ForeignKey(User, on_delete=models.CASCADE, null=True, blank=True)
    name = models.CharField(max_length=100)
    email = models.EmailField()
    level = models.IntegerField(default=User.LEVEL1, choices=User.LEVELS)
    active = models.BooleanField(default=True)

    hourly_cost_value = models.DecimalField(max_digits=15, decimal_places=2, default=0, null=True, blank=True)
    hourly_sell_value = models.DecimalField(max_digits=15, decimal_places=2, default=0, null=True, blank=True)
...
class CustomerGroup(models.Model):
    name = models.CharField(max_length=100)
    customers = models.ManyToManyField('core.Customer', blank=True,)
