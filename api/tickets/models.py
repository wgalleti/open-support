...
class Attachment(models.Model):
    ...
    user = models.ForeignKey(
        to="core.User", null=True, blank=True, on_delete=models.CASCADE
    )
...
class Ticket(TimeModelMixin, UserModelMixin):
    ...
    attendant = models.ForeignKey(
        "core.Attendant", on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    files = models.ManyToManyField(to="tickets.Attachment", blank=True)
...
class TicketInteraction(TimeModelMixin, UserModelMixin):
    ...
    status = models.CharField(
        max_length=50, choices=Ticket.STATUS, default=Ticket.WORKING
    )
    departament = models.CharField(
        max_length=50, choices=Ticket.DEPARTMENTS, default=Ticket.SUPPORT
    )
    files = models.ManyToManyField(to="tickets.Attachment", blank=True)
